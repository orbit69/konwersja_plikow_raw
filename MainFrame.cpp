...
MainFrame::MainFrame( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxFrame( parent, id, title, pos, size, style )
{
	...
	optionButtons[2] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("Use wavelets to erase noise while preserving real detail. The best threshold should be somewhere between 100 and 1000."), wxString("-n noise_threshold"), wxPoint(1, 60));
	optionButtons[3] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("Enlarge the raw red and blue layers by the given factors, typically 0.999 to 1.001, to correct chromatic aberration."), wxString("-c red_mag blue_mag"), wxPoint(1, 90));
	optionButtons[4] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("Calculate the white balance by averaging the entire image."), wxString("-a"), wxPoint(1, 120));
	optionButtons[5] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("Select the output colorspace when the -p option is not used:\n0   Raw color(unique to each camera)\n1   sRGB D65(default)\n2   Adobe RGB(1998) D65\n3   Wide Gamut RGB D65\n4   Kodak ProPhoto RGB D65\n5   XYZ\n6   ACES"), wxString("-o [0-6]"), wxPoint(1, 150));
	optionButtons[6] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("Show the raw data as a grayscale image with no interpolation. Good for photographing black-and-white documents."), wxString("-d"), wxPoint(1, 180));
	optionButtons[7] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("Output a half-size color image."), wxString("-h"), wxPoint(1, 210));
	optionButtons[8] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("0  Use high-speed, low-quality bilinear interpolation.\n1  Use Variable Number of Gradients (VNG) interpolation.\n2  Use Patterned Pixel Grouping (PPG) interpolation.\n3  Use Adaptive Homogeneity-Directed (AHD) interpolation."), wxString("-q [0-3]"), wxPoint(1, 240));
	optionButtons[9] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("Interpolate RGB as four colors. Use this if the output shows false 2x2 meshes with VNG or mazes with AHD."), wxString("-f"), wxPoint(1, 270));
	optionButtons[10] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("Use a fixed white level, ignoring the image histogram."), wxString("-W"), wxPoint(1, 300));
	optionButtons[11] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("Divide the white level by this number, 1.0 by default."), wxString("-b brightness"), wxPoint(1, 330));
	optionButtons[12] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("Set the gamma curve, by default BT.709 (-g 2.222 4.5). If you prefer sRGB gamma, use -g 2.4 12.92. For a simple power curve, set the toe slope to zero."), wxString("-g power toe_slope"), wxPoint(1, 360));
	optionButtons[13] = new ToggleTooltipButton(SideBar, ID_OPTION_BUTTON2, wxString("Flip the output image. By default, dcraw applies the flip specified by the camera. -t 0 disables all flipping."), wxString("-t [0-7,90,180,270]"), wxPoint(1, 390));
